...
public class ArenaCommand implements CommandExecutor, TabCompleter {
    ...
    public ArenaCommand(TheTowers theTowers) {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String @NotNull [] args) {
        if (sender instanceof Player) {
            ...
            if (args.length == 1 && args[0].equalsIgnoreCase("list")) {
                ...
                boolean result = theTowers.getArenaManager().leave(player);
                if (result) {
                ...
                SpectateResult result = theTowers.getArenaManager().spectate(player, arena);
                if (result != SpectateResult.OK) {
                    player.sendMessage(Component.translatable(result));
                }
            } else if (args.length == 1 && args[0].equalsIgnoreCase("reload")) {
                if (!player.hasPermission("thetowers.admin")) {
                    player.sendMessage(Component.translatable("command.arena.no-permission"));
                    return true;
                }

                int count = LangManager.load(theTowers);
                theTowers.getSignManager().refreshAll();
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String @NotNull [] args) {
        if (args.length == 1) {
            ...
            if (sender.hasPermission("thetowers.admin")) options.addAll(ADMIN_SUBCOMMANDS);
